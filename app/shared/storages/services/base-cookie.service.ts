...
export class BaseCookieService implements CookieService {
  ...
  getCookieString(): string | any {
    return this.documentIsAccessible ? document.cookie : this.request.headers.cookie;
  }
  ...
  get(name: string): string | any {
    if (this.check(name)) {
      ...
      const regExp: RegExp = this.getCookieRegExp(name);
      const result: RegExpExecArray | any = regExp.exec(this.getCookieString());
